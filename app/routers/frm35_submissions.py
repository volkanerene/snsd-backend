"""FRM35 Submissions - Quality Management Form"""
...
router = APIRouter(prefix="/frm35", tags=["FRM35 Submissions"])


@router.get("/submissions")
async def list_frm35_submissions(
    ...
    """List FRM35 submissions"""
    query = (
        supabase.table("frm35_submissions")
        .select("*")
...
async def create_frm35_submission(
    payload: dict = Body(...),
    user=Depends(get_current_user),
    tenant_id: str = Depends(require_tenant),
):
    """Create/update FRM35 submission"""
    ...
    try:
        res = supabase.table("frm35_submissions").insert(payload).execute()
...
async def update_frm35_submission(
    submission_id: str,
    payload: dict = Body(...),
    user=Depends(get_current_user),
    tenant_id: str = Depends(require_tenant),
):
    """Update FRM35 submission"""
    if not payload:
...
async def submit_frm35(
    ...
    try:
        ...
        supabase.table("frm35_submissions").update({
            "status": "submitted",
            "submitted_at": now,
            "updated_at": now
        ...
        return {
            "success": True,
            "message": "FRM35 submitted successfully",
